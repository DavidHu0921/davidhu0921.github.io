...
* 提示：你可以通过选择一些代码（或⌘A全部选择），然后选择Control-I（或菜单中的Editor \ Structure \ Re-Indent）来重新缩进。 一些Xcode模板代码将有4个空格的标签硬编码，所以这是一个很好的方法来解决这个问题。
...
* 方法之间应该只有一条空白线，以帮助视觉清晰和组织。 方法中的空格应该分开功能，但方法中的部分太多通常意味着你应该重构几种方法。
...
始终使用Swift的本机类型。 Swift提供桥接到Objective-C，所以你仍然可以根据需要使用一整套方法。
...
使用用`！`声明的隐式解开的类型，只有你知道的变量将在使用前稍后初始化，例如将在“viewDidLoad”中设置的子视图。
